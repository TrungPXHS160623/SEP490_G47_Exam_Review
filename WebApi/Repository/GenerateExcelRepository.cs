        ...
        public byte[] GenerateExcel()
        {
            ...
            using (ExcelPackage package = new ExcelPackage())
            {
                ...
                foreach (var exam in sortedExams)
                {
                    ...
                    var report = exam.Reports.FirstOrDefault();
                    worksheet.Cells[row, 10].Value = report?.ReportContent ?? "N/A"; // Report Review

                    var solution = exam.Reports.FirstOrDefault();
                    worksheet.Cells[row, 11].Value = solution?.QuestionSolutionDetail ?? "N/A"; // Solution Details
